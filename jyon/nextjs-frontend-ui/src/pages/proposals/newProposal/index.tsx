...
import { POST_COMMUNITY } from "@/constants/PostCommunity";
...
const NewProposal = () => {
  ...
  const setCommunityFromId = (event: any, value: any) => {
	setValue("community", value.id);
	console.log(value);
  };
        ...
        >
          ...
          <NewProposalInputWrapper>
			...
			<Autocomplete
			  ...
			  disabled={isLoading || Boolean(postId)}
			  options={[
				{ label: "Federal", id: POST_COMMUNITY.FEDERAL },
				{ label: "State", id: POST_COMMUNITY.STATE },
				{ label: "Local", id: POST_COMMUNITY.LOCAL },
			  ]}
			  isOptionEqualToValue={(option, value) =>
				option.id === value.id
			  }
			  onChange={setCommunityFromId}
			  getOptionLabel={(option) => option.label || ""}
			  sx={{ width: "auto", marginTop: "4px" }}
			  renderInput={(params) => (
				<TextField
				  {...params}
				  placeholder={
					Boolean(postId) ? challenge?.title : "Select Community"
				  }
				  error={errors.challenge?.message ? true : false}
				  helperText={errors.challenge?.message}
                />
